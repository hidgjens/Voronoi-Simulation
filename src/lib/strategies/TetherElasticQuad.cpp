#include "TetherElasticQuad.h"

TetherElasticQuad::TetherElasticQuad() {}

TetherElasticQuad::TetherElasticQuad(TetherElasticQuad& t) : 
...
TetherElasticQuad& TetherElasticQuad::operator=(TetherElasticQuad& t) {
    if (&t == this) {
        return *this;
    }
    pm = t.pm;
...
TetherElasticQuad::TetherElasticQuad(TeamConfigFile tcf, PitchModel* p) : 
...
max_post_distance(tcf.max_post_distance)
{
    ...
    spring_constant = 2.5 / (max_post_distance * max_post_distance);
...
void TetherElasticQuad::updateTeam(Team& team, Frame frame){
...
void TetherElasticQuad::exchange_method(Player* plyr, Frame& frame){
...
double TetherElasticQuad::metric(double dist, double ctrl) {
    // std::cout << "Here " << pitch_data->getXlim() << std::endl;
    return ctrl * exp( - 1.0 * dist / (pitch_data.getPitchLength() * decay_coefficient));
    
}

double TetherElasticQuad::KR(double r) { return spring_constant * (r * r) ; }