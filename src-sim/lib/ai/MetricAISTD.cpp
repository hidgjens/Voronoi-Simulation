...
MetricAISTD::MetricAISTD() : decay_const(1.0), op_coeff(1.0), sm_coeff(1.0) {
  setDesc("Find nearest opponent with larger area and approach them in attempt to swap positions");
}

MetricAISTD::MetricAISTD(double d, double o, double s) : decay_const(d), op_coeff(o), sm_coeff(s) {
  setDesc("Find nearest opponent with larger area and approach them in attempt to swap positions");
}

void MetricAISTD::updateFrame(Player& plyr, Match& match){
...
Cart MetricAISTD::metricV(Player& test_plyr, Player& far_plyr, Match& match) {
...
double MetricAISTD::metricD(Player& test_plyr, Player& far_plyr, Match& match) {
